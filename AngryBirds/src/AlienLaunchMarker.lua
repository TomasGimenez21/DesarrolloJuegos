...
function AlienLaunchMarker:update(dt)
    ...
    if not self.launched then
        ...
        elseif #self.aliensClone==0 and not self.hit then
            if love.keyboard.wasPressed('space') then
                local x,y = self.alien.body:getPosition()
                table.insert(self.aliensClone, Alien(self.world, 'round', x, y - 50, 'Player'))
                self.aliensClone[1].body:setLinearVelocity((self.baseX - self.shiftedX) * 10, (self.baseY - self.shiftedY) * 10)
                self.aliensClone[1].fixture:setRestitution(0.4)
                self.aliensClone[1].body:setAngularDamping(1)
                table.insert(self.aliensClone, Alien(self.world, 'round', x, y + 30, 'Player'))
                self.aliensClone[2].body:setLinearVelocity((self.baseX - self.shiftedX) * 10, (self.baseY - self.shiftedY) * 10)
                self.aliensClone[2].fixture:setRestitution(0.4)
                self.aliensClone[2].body:setAngularDamping(1)
...
function AlienLaunchMarker:render()
    if not self.launched then
        ...
        self.alien:render()
        if #self.aliensClone>0 then
            self.aliensClone[1]:render()
            self.aliensClone[2]:render()
        end
    end
end